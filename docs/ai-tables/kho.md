...
| **Migration** | `supabase-nhap-kho-ma-qr-columns.sql` — cột QR cho `nhap_kho`; `supabase-phieu-nhap-xuat-metadata.sql` — metadata phiếu; `supabase-kho-slip-status.sql` — trạng thái `status` |
...
- **Lưu đợt** ghi QR vào `nhap_kho`/`xuat_kho` và tạo header `phieu_nhap`/`phieu_xuat` với `status='chua_chot'`.
