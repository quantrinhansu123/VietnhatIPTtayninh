...
function SimpleTable({ children, className = '' }: { children: React.ReactNode; className?: string }) {
  return <table className={`shift-print-v2-table ${className}`}><tbody>{children}</tbody></table>;
}

export function ShiftHandoverPrintSheetV2({ slip }: { slip: ShiftHandoverPrintSlip }) {
  const stock = slip.closingStockLines || [];
  const materials = (slip.materials || []).filter(line => line.materialCode || line.materialName);
  ...
  return (
    <article className="shift-print-v2-sheet">
      <header className="shift-print-v2-header">
        ...
        <SimpleTable className="shift-print-v2-control"><tr><th>Ký hiệu</th><td>{SHIFT_HANDOVER_FORM_CODE}</td></tr><tr><th>Lần BH</th><td>{SHIFT_HANDOVER_FORM_ISSUE}</td></tr><tr><th>Ngày HL</th><td>{SHIFT_HANDOVER_FORM_EFFECTIVE}</td></tr></SimpleTable>
      </header>
      ...
      <Section title="2. Bảng trộn vật tư"><table className="shift-print-v2-table shift-print-v2-materials"><thead><tr><th>STT</th><th className="left">Tên vật tư</th><th>ĐVT</th><th>Tỷ lệ ĐM (%)</th><th>Sử dụng (kg)<br /><small>L1 · L2 · L3 · L4 · L5</small></th><th>Tổng nhựa trộn</th></tr></thead><tbody>{materials.map((line, i) => { const total = line.mixedTotal ?? mixedTotalFromUses(line.use1, line.use2, line.use3, line.use4, line.use5); return <tr key={`${line.stt}-${i}`}><td>{i + 1}</td><td className="left">{line.materialName || line.materialCode}</td><td>{line.unit}</td><td>{num(line.percent, 1)}</td><td className="uses">{[line.use1, line.use2, line.use3, line.use4, line.use5].map((value, j) => <span key={j}><b>L{j + 1}</b> {num(value)}</span>)}</td><td>{num(total)}</td></tr>; })}<tr className="total"><td colSpan={5}>Tổng cộng</td><td>{num(materials.reduce((s, line) => s + (line.mixedTotal ?? mixedTotalFromUses(line.use1, line.use2, line.use3, line.use4, line.use5) ?? 0), 0))}</td></tr></tbody></table></Section>

      {products.length > 0 && <Section title="3. Thành phẩm"><table className="shift-print-v2-table"><thead><tr><th>STT</th><th className="left">Mã / tên sản phẩm</th><th>SL</th><th>TL cuộn</th><th>Định mức nhựa</th><th>Hàng lỗi 20%</th><th>Hàng lỗi 30%</th></tr></thead><tbody>{products.map((line, i) => <tr key={`${line.stt}-${i}`}><td>{i + 1}</td><td className="left">{line.productCode} — {line.productName}</td><td>{num(line.quantity)}</td><td>{num(line.rollWeight)}</td><td>{num(line.resinNorm)}</td><td>{line.defect20}</td><td>{line.defect30}</td></tr>)}</tbody></table></Section>}
      {scraps.length > 0 && <Section title="4. Hàng lỗi"><table className="shift-print-v2-table"><thead><tr><th>STT</th><th className="left">Nội dung</th><th>Số lượng</th></tr></thead><tbody>{scraps.map((line, i) => <tr key={`${line.stt}-${i}`}><td>{i + 1}</td><td className="left">{line.name}</td><td>{num(line.quantity)}</td></tr>)}</tbody></table></Section>}
      {kpis.length > 0 && <Section title="5. Chỉ tiêu / KPI"><table className="shift-print-v2-table"><thead><tr><th>STT</th><th className="left">Tiêu chí</th><th>Định mức</th><th>Thực tế</th><th>Chênh lệch</th></tr></thead><tbody>{kpis.map((line, i) => <tr key={`${line.stt}-${i}`}><td>{i + 1}</td><td className="left">{line.criteria}</td><td>{num(line.norm)}</td><td>{num(line.actual)}</td><td>{num(line.variance)}</td></tr>)}</tbody></table></Section>}
      {tasks.length > 0 && <Section title="6. Công việc / sự cố bàn giao"><table className="shift-print-v2-table"><thead><tr><th>STT</th><th className="left">Nội dung</th><th>Mức độ</th><th>Người phụ trách</th><th>Trạng thái</th><th>Ghi chú</th></tr></thead><tbody>{tasks.map((line, i) => <tr key={`${line.stt}-${i}`}><td>{i + 1}</td><td className="left">{line.content}</td><td>{line.priority}</td><td>{line.assignee}</td><td>{line.status}</td><td className="left">{line.note}</td></tr>)}</tbody></table></Section>}
