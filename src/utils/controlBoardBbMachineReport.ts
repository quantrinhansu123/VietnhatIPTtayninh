...
export const BB_MACHINE_REPORT_TABS: Array<{ id: BbMachineReportTabId; label: string }> = [
  ...
  { id: 'danh_gia_tong_hop', label: 'Báo cáo tổng hợp' },
...
export type BbProductionOrderLineRow = {
  ...
  /** TL nhựa thực tế (kg) — phiếu in mục 2 / máy cách nhiệt. */
  actualPlasticWeightKg?: number | null;
  /** TL màng thực tế (kg) — máy cách nhiệt. */
  actualFilmWeightKg?: number | null;
  /** % SL đạt / kế hoạch. */
  planQtyRatioPercent?: number | null;
};
...
/**
 * Σ Nhựa thực tế (Thành phẩm) từ bảng Cân AI — khớp ngày · ca · máy · mã SP.
 * = Cân SP − lõi − bì − màng; dùng làm gốc TL NVL % trên Báo cáo sản lượng.
 */
function sumCanTuDongThanhPhamNhuaKgForProduct(input: {
  ...
  const productKey = normalizeProductCodeKey(input.productCode);
  if (!productKey || productKey === '-') return 0;
  if (!Array.isArray(input.records) || input.records.length === 0) return 0;
  const scoped = filterCanTuDongRecordsForBoard(input.records, {
    shiftFilter: input.shift,
    dateFrom: input.ngay,
    dateTo: input.ngay,
    machineFilter: input.machine,
    productCodeKeys: [productKey]
  });
  if (scoped.length === 0) return 0;
  const totals = sumCanTuDongSanLuongTotals(scoped, input.filmKgByProductCode);
  return totals.weightKg > 0 ? totals.weightKg : 0;
}

/**
 * Vật tư trộn (ĐVT kg / %): TL = Tổng nhựa thành phẩm × tỉ lệ % — không nhân TL cả cuộn.
 * Tổng nhựa thành phẩm ưu tiên Σ Nhựa TT Cân AI; fallback trong_luong_nhua × SL SP.
 */
function applyBbSanLuongMixLineFromPlastic(
  line: BbSanLuongNvlLine,
  tongNhuaThanhPhamKg: number,
  bomItem: ProductNplItem | undefined
): BbSanLuongNvlLine {
  const isMix = bomItem
    ? bomItem.amountType === 'percent' || isBbSanLuongNvlKgLine(line)
    : isBbSanLuongNvlKgLine(line);
  if (!isMix) return line;
  if (bomItem && isBbSanLuongNvlPhu(bomItem.amountType)) return line;

  const rateFromLine =
    line.quantity != null && Number.isFinite(line.quantity) && line.quantity > 0
      ? line.quantity
      : line.dinhMucRate != null && line.dinhMucRate > 0
        ? line.dinhMucRate
        : line.tiLeDinhMucPercent != null && line.tiLeDinhMucPercent > 0
          ? line.tiLeDinhMucPercent
          : 0;
  const rateFromBom =
    bomItem?.percent != null && Number.isFinite(bomItem.percent) && bomItem.percent > 0
      ? bomItem.percent
      : 0;
  const useRate = rateFromLine > 0 ? rateFromLine : rateFromBom;
  if (!(useRate > 0) || !(tongNhuaThanhPhamKg > 0)) return line;

  const weightKg = tongNhuaThanhPhamKg * (useRate / 100);
  return {
    ...line,
    unit: line.unit === '%' || !String(line.unit || '').trim() ? '%' : line.unit,
    dinhMucUnit: '%',
    amountType: 'percent',
    quantity: useRate,
    dinhMucRate: useRate,
    tiLeDinhMucPercent: useRate,
...
export function enrichBbProductionOrderRowsFromSanLuong(
  ...
  if (sanLuongGroups.length === 0) {
    return merged.map(row => enrichBbProductionOrderLineWeights(row, products));
  }
  return merged.map(row => {
    const productGroup = findBbSanLuongProductForOrderLine(row, sanLuongGroups);
    if (!productGroup) {
      return enrichBbProductionOrderLineWeights(
        { ...row, actualQuantity: 0, actualWeightKg: null },
        products
      );
    }
...
function enrichBbProductionOrderLineWeights(
  row: BbProductionOrderLineRow,
  products?: ProductRow[]
): BbProductionOrderLineRow {
  const planQtyRatioPercent =
    row.quantity > 0 && Number.isFinite(row.actualQuantity)
      ? roundQty((row.actualQuantity / row.quantity) * 100, 4)
      : null;
  const product = products?.length
    ? findProductByCode(products, row.productCode) ||
      products.find(
        item => normalizeProductCodeKey(item.name) === normalizeProductCodeKey(row.productName)
      )
    : null;
  let actualPlasticWeightKg: number | null = null;
  let actualFilmWeightKg: number | null = null;
  if (product && row.actualQuantity > 0) {
    const plasticPer = resolveProductMaterialBaseKg(product);
    if (plasticPer > 0) {
      actualPlasticWeightKg = roundQty(plasticPer * row.actualQuantity, 4);
    }
    const filmPer = resolveInsulationFilmKgPerRoll(product);
    if (filmPer != null && filmPer > 0) {
      actualFilmWeightKg = roundQty(filmPer * row.actualQuantity, 4);
    }
  }
  return {
    ...row,
    planQtyRatioPercent,
    actualPlasticWeightKg,
    actualFilmWeightKg
  };
}
...
function mergeSanLuongNvlFromBomAndSnapshot(
  ...
  for (const bomItem of dedupedBom) {
    ...
    if (!snap || snap.hasSnapshot === false) {
      result.push({
        ...mapped,
        // NVL %: giữ % ĐM từ BOM để tính TL nhựa; NVL phụ: SL = SL SP.
        rate: mapped.rate,
        quantityBySl: isNvlPhu ? resolveBbSanLuongNvlPhuQuantity(productQty) : mapped.quantityBySl,
        hasSnapshot: false
      });
      continue;
    }
 ...
 * - Vật tư trộn (%/kg): TL = Tổng nhựa thành phẩm × % — ưu tiên weightKg snapshot (Σ Nhựa TT Cân AI),
 *   không có thì fallback trong_luong_nhua × SL; không nhân TL cuộn.
...
export function overlayProductNplOnBbSanLuongGroups(
  ...
  return groups.map(group => {
    ...
    const productGroups = sourceProductGroups.map(productGroup => {
      ...
      const slSp = productGroup.quantity > 0 ? productGroup.quantity : 0;
      const plasticPerSp = catalog ? resolveProductMaterialBaseKg(catalog) : 0;
      // Giữ Σ Nhựa TT Cân AI đã lưu trên snapshot; chỉ fallback ĐM khi chưa có.
      const tongNhuaThanhPhamKg =
        productGroup.weightKg > 0
          ? productGroup.weightKg
          : plasticPerSp > 0 && slSp > 0
            ? plasticPerSp * slSp
            : 0;
      ...
      const recalculatedLines = sourceLines.map(line => {
        ...
        const bomItem = catalog
          ...
          : undefined;
        const mixFixed = applyBbSanLuongMixLineFromPlastic(line, tongNhuaThanhPhamKg, bomItem);
        return applyBbSanLuongNvlPhuToLine(mixFixed, slSp, bomItem, materialsCatalog);
      });
      ...
      if (catalog?.nplItems?.length) {
        for (const item of dedupeProductNplItemsForSanLuong(catalog.nplItems)) {
          ...
          const rate = amountType === 'percent' ? resolveProductNplSanLuongRate(item) ?? 0 : 0;
          const quantity = isNvlPhu
            ? resolveBbSanLuongNvlPhuQuantity(slSp)
            : amountType === 'percent'
              ? rate
              : 0;
          const weightKg = isNvlPhu
            ...
            : amountType === 'percent' && tongNhuaThanhPhamKg > 0 && rate > 0
              ? tongNhuaThanhPhamKg * (rate / 100)
              : 0;
          ...
          missingLines.push({
            ...
            itemName: item.name || item.code || '',
            unit,
            tiLeDinhMucPercent: amountType === 'percent' ? rate : null,
            dinhMucRate: isNvlPhu ? resolveProductNplSanLuongRate(item) ?? 0 : rate,
      ...
      return {
        ...productGroup,
        // Cột / ô «Tổng vật tư trộn» = Tổng nhựa thành phẩm (không TL cả cuộn).
        weightKg: tongNhuaThanhPhamKg > 0 ? tongNhuaThanhPhamKg : productGroup.weightKg,
        lines,
        lineCount: lines.length,
 ...
 * - ĐVT kg / kiểu % (NNS, NTC…): Nhựa thành phẩm × tỉ lệ % — ưu tiên Σ Nhựa TT Cân AI;
 *   không có thì trong_luong_nhua × SL; không nhân TL cả cuộn phiếu.
...
function resolveBbSanLuongNvlLineWeightKg(input: {
  ...
  materialsCatalog: WarehouseWeightCatalogItem[];
  materials: MaterialRow[];
  /** Σ Nhựa TT Cân AI của đúng SP (ngày·ca·máy) — gốc TL NVL %. */
  thanhPhamNhuaKg?: number | null;
}): number {
  const {
    item,
    bomItem,
  ...
  if (isPercent) {
    const useRate =
      rate > 0
        ? rate
        : bomItem?.percent != null && bomItem.percent > 0
          ? bomItem.percent
          : 0;
    if (!(useRate > 0)) return 0;

    // Ưu tiên Σ Nhựa TT Cân AI (Thành phẩm); không thì nhựa/1 SP × SL.
    const plasticPerSp = resolveProductMaterialBaseKg(catalog);
    const catalogNhuaKg =
      plasticPerSp > 0 && inboundQty > 0 ? plasticPerSp * inboundQty : 0;
    const trongLuongNhuaKg =
      input.thanhPhamNhuaKg != null && input.thanhPhamNhuaKg > 0
        ? input.thanhPhamNhuaKg
        : catalogNhuaKg;
    if (trongLuongNhuaKg > 0) {
      return trongLuongNhuaKg * (useRate / 100);
    }

    // Fallback: khoi_luong_kg trên Thành phần (đã là phần nhựa/1 SP của NVL này)
    if (
      bomItem?.weightKg != null &&
      Number.isFinite(bomItem.weightKg) &&
      bomItem.weightKg >= 0 &&
      inboundQty > 0
    ) {
      return bomItem.weightKg * inboundQty;
    }
    return 0;
  }
...
export function buildBbSanLuongGroups(input: {
  ...
  /** Phiếu Cân AI — Σ Nhựa TT (Thành phẩm) làm gốc TL NVL %. */
  canTuDongRecords?: CanTuDongWeightRow[];
  shiftSettings?: (ShiftSetting | ProductionOrderLookupSetting)[];
  dateFrom: string;
  dateTo: string;
  ...
  const lookupSettings = (input.shiftSettings || []) as ProductionOrderLookupSetting[];
  const canTuDongRecords = Array.isArray(input.canTuDongRecords) ? input.canTuDongRecords : [];
  const filmKgByProductCode =
    canTuDongRecords.length > 0
      ? buildCanTuDongFilmKgByProductCode(input.products)
      : undefined;
  ...
  for (const bucket of buckets.values()) {
    ...
    for (const report of bucket.reports) {
      ...
      // TL SP trên báo cáo sản lượng = trọng lượng nhựa (+ phụ gia), không lấy TL cả cuộn.
      const plasticPerUnit = resolveProductMaterialBaseKg(catalog);
      const productWeightKg =
        plasticPerUnit > 0 && inboundQty > 0
          ? plasticPerUnit * inboundQty
          : reportWeightKg;
      ...
      for (const item of nvlItems) {
        ...
        // NVL %: luôn lấy % từ BOM/snapshot để tính TL nhựa; thiếu snapshot vẫn tính từ Thành phần.
        ...
        if (!existing) {
          ...
          // NVL nhựa trộn (%): không làm tròn khi cộng dồn.
          if (isPercent) {
    ...
    // TL NVL % + Tổng vật tư trộn: ưu tiên Σ Nhựa TT Cân AI (Thành phẩm) theo ngày·ca·máy·mã SP.
    for (const product of productMap.values()) {
      const canTuDongNhuaKg = sumCanTuDongThanhPhamNhuaKgForProduct({
      ...
      if (!(canTuDongNhuaKg > 0)) continue;

      product.weightKg = canTuDongNhuaKg;
      for (const nvl of product.nvlAgg.values()) {
        if (nvl.amountType !== 'percent') continue;
        if (!(nvl.rate > 0)) continue;
        const lineKg = canTuDongNhuaKg * (nvl.rate / 100);
        nvl.normWeightKg = lineKg;
        nvl.actualWeightKg = lineKg;
      }
    }
      ...
      .map(([productKey, product]) => {
        ...
        const lines: BbSanLuongNvlLine[] = orderBbSanLuongNvlLinesByKg(
          [...product.nvlAgg.entries()].map(([materialKey, nvl]) => ({
            ...
            unit: nvl.unit,
            tiLeDinhMucPercent:
              nvl.amountType === 'percent' ? nvl.rate : null,
            dinhMucRate: nvl.rate,
            dinhMucUnit: nvl.amountType === 'percent' ? '%' : nvl.unit || 'đơn vị',
            amountType: nvl.amountType,
            quantity: nvl.quantity,
            normWeightKg:
              nvl.amountType === 'percent' ? nvl.normWeightKg : roundQty(nvl.normWeightKg, 4),
            materialNorm: null,
            baseActualWeightKg:
              nvl.amountType === 'percent'
                ? nvl.actualWeightKg
                : roundQty(nvl.actualWeightKg, 4),
            productSharePercent,
            actualWeightKg:
              nvl.amountType === 'percent'
                ? nvl.actualWeightKg
...
export type BbThucDungLineRow = {
  ...
  /**
   * true = NVL nhựa (mục 3.1) — snapshot mới gán bằng isPlasticNvl.
   * Snapshot cũ: NVL trong bảng tỉ lệ trộn máy.
   */
  inMixingRatioTable: boolean;
  /** true = NVL nhựa (phiếu in 3.1). Snapshot cũ có thể thiếu → suy từ unit/mã. */
  isPlasticNvl?: boolean;
  /** Số lượng thành phẩm (ĐVT gốc) — mục 3.2. */
  finishedQty?: number;
  /** SL xuất kho (ĐVT gốc). */
  exportQty?: number;
};
...
/**
 * Nhập TP / lỗi / TP+lỗi — lấy số đã lưu trên dòng (snapshot Tính toán).
 * Không tính lại từ banner × tỉ lệ (phiếu in chỉ mirror tab).
 */
export function resolveBbThucDungKlNhuaTtLoiKg(
  line: Pick<BbThucDungLineRow, 'klThucTeKg' | 'loiHongKg' | 'klThucTePlusLoiKg'>,
  ...
  const klThucTePlusLoiKg =
    Number.isFinite(line.klThucTePlusLoiKg) && (line.klThucTePlusLoiKg as number) !== 0
      ? (line.klThucTePlusLoiKg as number)
      : roundQty(klThucTeKg + loiHongKg, 4);
...
export function buildBbThucDungLineRows(input: {
  ...
  for (const header of headers) {
    ...
    /** NVL seed từ Thành phần SP trên lệnh — giữ dòng dù chưa có xuất/tồn. */
    const fromProductBomKeys = new Set<string>();
    ...
    // Seed đủ NVL từ Thành phần SP (npl_phan_tram) của lệnh.
    for (const order of input.productionOrders) {
      if (!isBbProductionOrder(order, input.machines, input.includeAllMachines)) continue;
      if (order.code !== header.orderCode) continue;
      const ngay = parseProductionOrderFilterDate(order.startDate);
      if (ngay !== header.ngay) continue;
      if (!shiftNamesMatch(order.shift, header.shift, shiftOptions)) continue;
      for (const line of getProductionOrderProductLines(order)) {
        const catalog = findProductByCode(input.products || [], String(line.productCode || '').trim());
        for (const item of catalog?.nplItems || []) {
          const code = String(item.code || '').trim();
          const name = String(item.name || '').trim();
          if (!code && !name) continue;
          const unit = String(item.unit || '').trim() || 'kg';
          const key = ensureMaterialAgg(code, name, unit);
          if (key) fromProductBomKeys.add(key);
        }
      }
    }
    ...
    for (const [materialKey, agg] of byMaterial.entries()) {
      ...
      // Giữ: tỉ lệ trộn máy · Thành phần SP · hoặc đã có số lượng.
      if (!inMixingRatioTable && !hasAnyQty && !fromProductBomKeys.has(materialKey)) {
        continue;
      }
      headerRows.push({
...
export function syncBbThucDungRowsTonCuoiFromCuoiCaTab(input: {
  ...
  return input.rows.map(row => {
    ...
    const finishedKg = Number.isFinite(row.klThucTeKg) ? row.klThucTeKg : 0;
    const damagedKg = Number.isFinite(row.loiHongKg) ? row.loiHongKg : 0;
    const klThucTePlusLoiKg = roundQty(finishedKg + damagedKg, 4);
    // Phiếu in: Chênh lệch = xuất thực dùng − nhập TP − lỗi hỏng.
    const chenhLechKg = roundQty(weightKg - finishedKg - damagedKg, 4);
    return {
      ...row,
      tonCuoiKg,
      directTonCuoiKg,
...
function isBbThucDungPlasticSectionRow(row: BbThucDungLineRow) {
  if (typeof row.isPlasticNvl === 'boolean') return row.isPlasticNvl;
  return Boolean(row.inMixingRatioTable);
}
...
function addBbThucDungSectionTotals(target: BbThucDungSectionTotals, row: BbThucDungLineRow) {
  ...
  target.finishedTotal += Number.isFinite(row.klThucTeKg) ? row.klThucTeKg : 0;
  target.damagedTotal += Number.isFinite(row.loiHongKg) ? row.loiHongKg : 0;
...
export function groupBbThucDungLines(rows: BbThucDungLineRow[]): BbThucDungGroup[] {
  ...
  for (const row of rows) {
    ...
    if (!existing) {
      ...
      if (isBbThucDungPlasticSectionRow(row)) addBbThucDungSectionTotals(mixingRatioTotals, row);
    ...
    existing.thucTronTotal +=
      ...
      Number.isFinite(row.mixingShiftMaterialKg)
        ? row.mixingShiftMaterialKg
        : 0;
    if (isBbThucDungPlasticSectionRow(row)) addBbThucDungSectionTotals(existing.mixingRatioTotals, row);
  ...
  return [...map.values()]
    .map(group => ({
      ...
      lines: [...group.lines].sort((a, b) => {
        const aPlastic = isBbThucDungPlasticSectionRow(a);
        const bPlastic = isBbThucDungPlasticSectionRow(b);
        if (aPlastic !== bPlastic) return aPlastic ? -1 : 1;
        return a.materialName.localeCompare(b.materialName, 'vi');
      })
    }))
    .sort((a, b) => {
...
export type BbDanhGiaHaoHutGroup = {
  ...
  /** Bảng 4.1 Tổng hợp — phiếu in chỉ mirror. */
  summaryRows?: BbDanhGiaSummaryRow[];
};

/** Một dòng bảng Báo cáo tổng hợp — khớp mẫu Excel (phiếu in / tab). */
export type BbDanhGiaSummaryRow = {
  id: 'loi_tren_tp' | 'loi_tren_tp_va_loi' | 'mang_loi_tren_tp' | 'mang_loi_tren_tp_va_loi' | 'nvl' | 'tong';
  /** Vật tư trộn · Các vật tư còn lại. Snapshot cũ thiếu → suy từ `id`. */
  section?: 'tron' | 'con_lai';
  /** Cột STT trên mẫu = mã NVL (không phải số thứ tự). */
  sttCode?: string;
  materialCode?: string;
  materialName?: string;
  label: string;
...
export function splitBbDanhGiaSummaryRowsBySection(rows: BbDanhGiaSummaryRow[] | undefined): {
  mixingRows: BbDanhGiaSummaryRow[];
  otherRows: BbDanhGiaSummaryRow[];
} {
  const list = rows || [];
  const mixingRows: BbDanhGiaSummaryRow[] = [];
  const otherRows: BbDanhGiaSummaryRow[] = [];
  for (const row of list) {
    const legacyId = String(row.id || '');
    const resolved: 'tron' | 'con_lai' =
      row.section || (legacyId === 'hao_hut_mang' ? 'con_lai' : 'tron');
    if (resolved === 'con_lai') otherRows.push(row);
    else mixingRows.push(row);
  }
  return { mixingRows, otherRows };
}

/** Dòng tỉ lệ lỗi hỏng / màng lỗi hỏng (chỉ số %) vs phân tích chi tiết NVL / Tổng. */
export function splitBbDanhGiaSummaryRowsRatioVsDetail(rows: BbDanhGiaSummaryRow[] | undefined): {
  ratioRows: BbDanhGiaSummaryRow[];
  detailRows: BbDanhGiaSummaryRow[];
} {
  const ratioRows: BbDanhGiaSummaryRow[] = [];
  const detailRows: BbDanhGiaSummaryRow[] = [];
  for (const row of rows || []) {
    if (
      row.id === 'loi_tren_tp' ||
      row.id === 'loi_tren_tp_va_loi' ||
      row.id === 'mang_loi_tren_tp' ||
      row.id === 'mang_loi_tren_tp_va_loi'
    ) {
      ratioRows.push(row);
    } else {
      detailRows.push(row);
  ...
  return { ratioRows, detailRows };
}

/** Map định mức BOM (kg) theo mã/tên NVL = kg/SP × SL nhập TP. */
export function buildBbBomDinhMucKgByMaterial(input: {
  order: BbProductionOrderGroup | undefined;
  products: ProductRow[];
  materials: MaterialRow[];
  sanLuongGroup?: BbSanLuongGroup | null;
}): Map<string, number> {
  const byKey = new Map<string, number>();
  const order = input.order;
  if (!order) return byKey;
  const materialsCatalog = input.materials.map(mapMaterialToWeightCatalogItem);
  const add = (code: string, name: string, kg: number) => {
    ...
    const key =
      ...
      String(code || name || '')
        ...
        .toUpperCase();
    if (!key) return;
    byKey.set(key, roundQty((byKey.get(key) || 0) + kg, 4));
  };

  for (const productLine of order.lines || []) {
    const product = findProductByCode(input.products, productLine.productCode);
    if (!product) continue;
    const codeKey = normalizeProductCodeKey(productLine.productCode);
    const fromSanLuong = (input.sanLuongGroup?.productGroups || []).find(
      pg => normalizeProductCodeKey(pg.productCode) === codeKey
    );
    const actualQty =
      fromSanLuong && fromSanLuong.quantity > 0
        ? fromSanLuong.quantity
        : productLine.actualQuantity > 0
          ? productLine.actualQuantity
    ...
    for (const item of product.nplItems || []) {
      const perSpKg = resolveProductNplItemWeightKg(product, item, input.materials);
...
export function lookupBbBomDinhMucKg(
...
function emptyBbDanhGiaSummaryMetricFields(): Pick<
  BbDanhGiaSummaryRow,
...
function buildBbDanhGiaSummaryNvlRow(input: {
  line: BbThucDungLineRow;
  section: 'tron' | 'con_lai';
  bomDinhMucKg: number;
  donGia: number;
  ...
  const tiLePercent =
    line.tiLeDinhMucPercent != null &&
    Number.isFinite(line.tiLeDinhMucPercent) &&
    line.tiLeDinhMucPercent > 0
      ? line.tiLeDinhMucPercent
      : line.tiLeThucTeTbPercent != null &&
          Number.isFinite(line.tiLeThucTeTbPercent) &&
  ...
  const thucXuatKg =
    section === 'tron' && tiLePercent != null && tongXuatThucDung > 0
      ? roundQty(tongXuatThucDung * (tiLePercent / 100), 2)
...
function buildBbDanhGiaSummaryTongRow(
  nvlRows: BbDanhGiaSummaryRow[],
  section: 'tron' | 'con_lai'
): BbDanhGiaSummaryRow {
  const sum = (pick: (row: BbDanhGiaSummaryRow) => number | null | undefined) =>
    roundQty(
      nvlRows.reduce((acc, row) => {
        const v = pick(row);
...
/**
 * Gắn bảng Báo cáo tổng hợp (summaryRows) theo mẫu Excel:
 * tỉ lệ lỗi · từng NVL Thành phần · Tổng — tách Vật tư trộn / Các vật tư còn lại.
 */
export function enrichBbDanhGiaGroupsWithPrintSummary(input: {
  ...
  const products = input.products || [];
  const materials = input.materials || [];
  const warehouseMovements = input.warehouseMovements || [];
  const shiftSettingsTyped = (input.shiftSettings || []) as ShiftSetting[];
  const findThucDung = (orderCode: string, groupKey: string) =>
    input.thucDungGroups.find(g => g.groupKey === groupKey || g.orderCode === orderCode);
  const findDamaged = (orderCode: string, groupKey: string) =>
    input.damagedGroups.find(g => g.groupKey === groupKey || g.orderCode === orderCode);
  const findOrder = (orderCode: string, groupKey: string) =>
    input.orderGroups.find(g => g.groupKey === groupKey || g.orderCode === orderCode);
  const findSanLuong = (orderCode: string, groupKey: string) =>
    (input.sanLuongGroups || []).find(g => g.groupKey === groupKey || g.orderCode === orderCode);

  return input.danhGiaGroups.map(group => {
    const thucDung = findThucDung(group.orderCode, group.groupKey);
    const damaged = findDamaged(group.orderCode, group.groupKey);
    const order = findOrder(group.orderCode, group.groupKey);
    const sanLuong = findSanLuong(group.orderCode, group.groupKey);
    const isInsulation = isInsulationMachineText(
      group.machine,
      input.selectedMachine?.name,
      input.selectedMachine?.code
    );
    const hangLoiNhua = resolveBbDamagedPlasticLoiHongKg(damaged?.lines || [], {
      isInsulationMachine: isInsulation
    });
    const hangLoiMang = resolveBbDamagedOtherLoiHongKg(damaged?.lines || [], {
      isInsulationMachine: isInsulation
    });
    ...
    const hangLoiNhuaKg = hangLoiNhua > 0 ? hangLoiNhua : 0;
    const hangLoiMangKg = hangLoiMang > 0 ? hangLoiMang : 0;
    const hangLoiTongKg = hangLoiNhuaKg + hangLoiMangKg;
    const thanhPhamTongKg = tongNhuaThanhPham + tongMangThanhPham;

    const bomDinhMucByMaterial = buildBbBomDinhMucKgByMaterial({
      order,
      products,
      materials,
      sanLuongGroup: sanLuong
    });

    const resolveDonGia = (line: BbThucDungLineRow) =>
    ...
    const tongNhuaThucXuatKg =
      group.tongNhuaThucXuat > 0
        ? roundQty(group.tongNhuaThucXuat, 4)
        : roundQty(
            plasticLines.reduce(
              (sum, row) => sum + (Number.isFinite(row.weightKg) ? row.weightKg : 0),
              0
            ),
            4
          );

    const plasticNvlRows = plasticLines.map(line =>
      buildBbDanhGiaSummaryNvlRow({
        line,
        section: 'tron',
    ...
    const summaryRows: BbDanhGiaSummaryRow[] = [
      ...
      ...otherNvlRows,
      ...(otherNvlRows.length > 0
        ? [buildBbDanhGiaSummaryTongRow(otherNvlRows, 'con_lai')]
        : [])
    ];
