/**
 * du_lieu_trong_bao_cao_kiem_ton_cuoi_ca — snapshot tab kiểm tồn cuối ca.
 * Khác bao_cao_may_nvl_ton. Ghi khi Tính toán.
 */
import type { BbCuoiCaGroup } from './controlBoardBbMachineReport';

export type DuLieuTrongBaoCaoKiemTonCuoiCaRow = {
  khoa_on_dinh: string;
  ngay: string;
  ca: string;
  ...
  ton_cuoi_sl: number | null;
  ton_cuoi_kg: number | null;
  tu_nns_tron: boolean | null;
  nns_tron_ton_cuoi_kg: number | null;
  ton_cuoi_truc_tiep_kg: number | null;
...
export function buildDuLieuTrongBaoCaoKiemTonCuoiCaRowsFromGroups(input: {
  khoaOnDinh: string;
  cuoiCaGroups: BbCuoiCaGroup[];
}): DuLieuTrongBaoCaoKiemTonCuoiCaRow[] {
  const khoa = String(input.khoaOnDinh || '').trim();
  if (!khoa) return [];

  const rows: DuLieuTrongBaoCaoKiemTonCuoiCaRow[] = [];
  for (const group of input.cuoiCaGroups || []) {
      ...
      group.totalWeightKg > 0
        ...
        : round4(
            materialLines.reduce(
              (sum, line) => sum + (line.tonDauWeightKg > 0 ? line.tonDauWeightKg : 0),
              0
            )
          );
    ...
    if (materialLines.length === 0) {
      rows.push({
        ...
        ton_cuoi_kg: null,
        tu_nns_tron: null,
        nns_tron_ton_cuoi_kg: null,
        ton_cuoi_truc_tiep_kg: null,
        so_dong_nvl: soDong,
        so_sp: soSp,
        tong_ton_cuoi_lenh_kg: tongTon > 0 ? tongTon : null
      });
      continue;
    }
    ...
    materialLines.forEach((line, index) => {
      ...
      rows.push({
        ...
        ton_cuoi_sl: numOrNull(line.tonDauQuantity),
        ton_cuoi_kg: numOrNull(line.tonDauWeightKg),
        tu_nns_tron: formula ? Boolean(formula.fromNnsTron) : null,
        nns_tron_ton_cuoi_kg: formula ? numOrNull(formula.nnsTronTonDauKg) : null,
        ton_cuoi_truc_tiep_kg: formula ? numOrNull(formula.directTonDauKg) : null,
        so_dong_nvl: soDong,
        so_sp: soSp,
        tong_ton_cuoi_lenh_kg: tongTon > 0 ? tongTon : null
      });
